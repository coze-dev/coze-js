...
describe('Error classes', () => {
  ...
  describe('APIError', () => {
    ...
    it('should set properties correctly', () => {
      ...
      const headers = new Headers({ 'x-tt-logid': '12345' });
    ...
    it('should create correct error message', () => {
      ...
      expect(error.message).toContain('help doc: https://www.coze.com/docs/error/1001');
    });
...
describe('APIError.generate', () => {
  it('should return APIConnectionError when status is undefined', () => {
    const error = APIError.generate(undefined, { code: 1001, msg: 'Error' }, 'Connection error', undefined);
    expect(error).toBeInstanceOf(APIConnectionError);
    expect(error.message).toBe('Connection error.');
  });

  it('should return BadRequestError for status 400', () => {
    const error = APIError.generate(400, { code: 4000, msg: 'Bad Request' }, undefined, undefined);
    expect(error).toBeInstanceOf(BadRequestError);
    expect(error.status).toBe(400);
  });

  it('should return AuthenticationError for status 401', () => {
    const error = APIError.generate(401, { code: 4100, msg: 'Unauthorized' }, undefined, undefined);
    expect(error).toBeInstanceOf(AuthenticationError);
    expect(error.status).toBe(401);
  });

  it('should return PermissionDeniedError for status 403', () => {
    const error = APIError.generate(403, { code: 4101, msg: 'Forbidden' }, undefined, undefined);
    expect(error).toBeInstanceOf(PermissionDeniedError);
    expect(error.status).toBe(403);
  });

  it('should return NotFoundError for status 404', () => {
    const error = APIError.generate(404, { code: 4200, msg: 'Not Found' }, undefined, undefined);
    expect(error).toBeInstanceOf(NotFoundError);
    expect(error.status).toBe(404);
  });

  it('should return RateLimitError for status 429', () => {
    const error = APIError.generate(429, { code: 4013, msg: 'Too Many Requests' }, undefined, undefined);
    expect(error).toBeInstanceOf(RateLimitError);
    expect(error.status).toBe(429);
  });

  it('should return GatewayError for status 502', () => {
    const error = APIError.generate(502, { code: 502, msg: 'Bad Gateway' }, undefined, undefined);
    expect(error).toBeInstanceOf(GatewayError);
    expect(error.status).toBe(502);
  });

  it('should return InternalServerError for status 500', () => {
    const error = APIError.generate(500, { code: 500, msg: 'Internal Server Error' }, undefined, undefined);
    expect(error).toBeInstanceOf(InternalServerError);
    expect(error.status).toBe(500);
  });

  it('should return generic APIError for unrecognized status', () => {
    const error = APIError.generate(418, { code: 418, msg: "I'm a teapot" }, undefined, undefined);

...
export interface TranscriptionDemoRef {
  isRecording: boolean;
  handlePauseAndResume: () => void;
}
...
const TranscriptionDemo = forwardRef<
  TranscriptionDemoRef,
  TranscriptionDemoProps
>(
  (
    {
      useByTTS = false,
    ...
    return (
      <Layout style={{ height: '100%' }}>
        <Settings
          ...
          fields={['base_ws_url', 'pat'].concat(
            useByTTS ? ['bot_id', 'voice_id', 'workflow_id'] : [],
          )}
        ...
        <Layout.Content style={{ background: '#fff', padding: '20px' }}>
          ...
          <Card title="录音控制" style={{ marginBottom: '20px' }}>
            ...
            <Row gutter={16}>
              ...
              {!isRecording && useByTTS && (
                <Col>
                  <Button
                    disabled={!transcriptionText}
                    onClick={() => {
                      setTranscriptionText('');
                      onSendMessage(transcriptionText);
                    }}
                    size="large"
                  >
                    发送至TTS

...
import { Layout, Card, Space, Typography, Button, message, Tag } from 'antd';
import { SoundOutlined, SoundFilled } from '@ant-design/icons';
import { useLocation } from 'react-router-dom';
import { WsToolsUtils } from '@coze/api/ws-tools';
import { COZE_CN_BASE_URL, CozeAPI, StreamInfo } from '@coze/api';
import { ConsoleLog } from '../../components/console-log';
import webRTCService, { ResourceStatus } from './webrtc-service';
...
function Subscribe() {
  ...
  useEffect(() => {
    ...
    const fetchLiveInfo = async () => {
      try {
        const api = new CozeAPI({
          ...
          token: '', // 免登录
          headers: {
            'X-TT-ENV': 'ppe_simul_interp',
            'x-use-ppe': '1',
          },
        });
        const data = await api.audio.live.retrieve(liveId);
        setStreamInfos(data.stream_infos);
    ...
    webRTCService.addStatusListener(setStatus);

    // 组件卸载时清理资源
    return () => {
      // 如果已连接，断开连接
      if (status === ResourceStatus.CONNECTED) {
        handleUnsubscribe();
      }
      webRTCService.removeStatusListener(setStatus);
    };
  }, []);
  ...
  const handleSubscribe = async (streamId: string) => {
    ...
    try {
      ...
      // 创建WebRTC订阅
      await webRTCService.subscribe(appId, streamId, clientId);
    } catch (error) {
          ...
          <Tag color={
            status === ResourceStatus.CONNECTED 
              ...
              : status === ResourceStatus.CONNECTING 
                ? 'processing' 
                : status === ResourceStatus.FAILED || status === ResourceStatus.CLOSED 
                  ? 'error' 
                  : 'default'
          }>
            {status === ResourceStatus.CONNECTED 

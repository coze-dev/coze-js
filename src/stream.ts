class Stream<T> {
  ...
  private handler: (event: string, data: string) => T;

  constructor(stream: ReadableStream, handler: (event: string, data: string) => T) {
    this.stream = stream;

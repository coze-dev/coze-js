...
function getBarHeights(waveBarNumber: number, volumeRealNumber: number) {
  ...
  const waveBarHeights = new Array(waveBarNumber)
    .fill(0)
    .map((_item, index) =>
      getBarHeight(index, waveBarNumber, volumeRealNumber),
    );
...
function getBarHeight(index: number, maxNumber: number, volumeNumber: number) {
  ...
  if (percent < 1 / 6) {
    ...
    randomMin = 0.1 + (Number(-0.8) - 0.1) * percent * 6;
...
function getBarBgColor(index: number, maxNumber: number, type: string) {
  let bgColor = '#FFF';
  switch (type) {
    case 'primary':
      {
